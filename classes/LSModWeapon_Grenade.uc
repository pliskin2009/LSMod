...
* Weapon: Smoke Pot
*/
class LSModWeapon_Grenade extends AOCWeapon_ThrowingAxe;
...
simulated function SpawnProjectile( Vector RealStartLoc, float Pitch, float Yaw, float Roll )
{
	...
	{
		`logalways("Rejected SpawnProjectile from"@AOCOWner.PlayerReplicationInfo.PlayerName@"because"@(Worldinfo.TimeSeconds - fLastProjectileSpawnTime)@"is faster than"@fReasonableRefireRate);
		return;
	}
	else
	{
		`logalways("Allowed SpawnProjectile from"@AOCOWner.PlayerReplicationInfo.PlayerName@"because"@(Worldinfo.TimeSeconds - fLastProjectileSpawnTime)@"is slower than"@fReasonableRefireRate);
	}	
...
DefaultProperties
{
	CurrentWeaponType = EWEP_SmokePot
	// set maximum ammo
	AmmoCount=2
	MaxAmmoCount=2
	AIRange=5000

	WeaponProjectiles(0)=class'LSModProj_Grenade3'

	AttachmentClass=class'AOCWeaponAttachment_SmokePot'
	// should never be part of inventory attachment
	InventoryAttachmentClass=class'AOCInventoryAttachment_SmokePot'

	ProjectileSpawnLocation=ProjPoint
	bCanPickupAmmoOffCorpse=false

	/* 
	 * Formerly in UDKNewWeapon.ini - [AOC.AOCWeapon_SmokePot]
	 */
	...
	ConfigProjectileBaseDamage[5]=(Damage=30,InitialSpeed=1550.0,MaxSpeed=1650.0,AmmoCount=2,InitialGravityScale=1.0,Drag=0.000001,PitchCorrection=60.0)
	iFeintStaminaCost=0
	WeaponFontSymbol="*"
	WeaponReach=100
	WeaponLargePortrait="SWF.weapon_select_smokepot"
	WeaponSmallPortrait="SWF.weapon_select_smokepot"
	...
	WindupAnimations(0)=(AnimationName=3p_1hsharp_throwdowntoup,ComboAnimation=,AssociatedSoundCue=SoundCue'A_Combat_Locomotion.broadsword_windup',bFullBody=False,bCombo=False,bLoop=False,bForce=false,fModifiedMovement=1.0,fAnimationLength=.6,fBlendInTime=0.00,fBlendOutTime=0.00,bLastAnimation=false)
	...
	RecoveryAnimations(0)=(AnimationName=3p_1hsharp_throwrecover,ComboAnimation=3p_1hsharp_slash011recover,AssociatedSoundCue=,bFullBody=false,bCombo=false,bLoop=false,bForce=false,UniqueShieldSound=none,fModifiedMovement=1.0,fAnimationLength=0.0,fBlendInTime=0.00,fBlendOutTime=0.0,bLastAnimation=true)
	RecoveryAnimations(1)=(AnimationName=3p_1hsharp_slash02recover,ComboAnimation=3p_1hsharp_slash021recover,AssociatedSoundCue=,bFullBody=false,bCombo=false,bLoop=false,bForce=false,UniqueShieldSound=none,fModifiedMovement=1.0,fAnimationLength=0.0,fBlendInTime=0.10,fBlendOutTime=0.1,bLastAnimation=true)
