...
class LSModWeapon_ThrowingMAUL extends AOCWeapon_ThrowingAxe;
...
simulated function SpawnProjectile( Vector RealStartLoc, float Pitch, float Yaw, float Roll )
{
	...
	if ( SpawnedProjectile != None )
	{
		...
		AOCOwner.CurrentWeaponAttachment.Mesh.GetSocketWorldLocationAndRotation('TraceStart', LSModProjectile(SpawnedProjectile).LongTraceStart,, 1);
		AOCOwner.CurrentWeaponAttachment.Mesh.GetSocketWorldLocationAndRotation('TraceEnd', LSModProjectile(SpawnedProjectile).LongTraceEnd,, 1);
	}
}
...
DefaultProperties
{
	...
	WeaponProjectiles(0)=class'LSModProj_ThrownMAUL'

	AttachmentClass=class'LSModWeaponAttachment_ThrownMAUL'
	...
	ConfigProjectileBaseDamage[5]=(Damage=40.0,InitialSpeed=2300.0,MaxSpeed=2800.0,AmmoCount=1,InitialGravityScale=0.85,Drag=0.0000045,PitchCorrection=0)
	iFeintStaminaCost=0
	WeaponFontSymbol="F"
	WeaponLargePortrait="SWF.weapon_select_maul"
	WeaponSmallPortrait="SWF.weapon_select_maul"
	...
	WindupAnimations(0)=(AnimationName=3p_1hsharp_throwdowntoup,ComboAnimation=,AssociatedSoundCue=SoundCue'A_Combat_Locomotion.doubleaxe_windup',bFullBody=False,bCombo=False,bLoop=False,bForce=false,fModifiedMovement=1.0,fAnimationLength=0.5,fBlendInTime=0.00,fBlendOutTime=0.00,bLastAnimation=false)
	...
	RecoveryAnimations(0)=(AnimationName=3p_1hsharp_throwrecover,ComboAnimation=3p_1hsharp_slash011recover,AssociatedSoundCue=,bFullBody=false,bCombo=false,bLoop=false,bForce=false,UniqueShieldSound=none,fModifiedMovement=1.0,fAnimationLength=0.425,fBlendInTime=0.00,fBlendOutTime=0.0,bLastAnimation=true)
